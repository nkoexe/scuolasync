...
class Updater:
    ...
    def get_local_version(self):
        ...
        if result.returncode != 0:
            ...
            if "No names found" in stderr or "no tags" in stderr.lower():
                ...
                return "untagged"
            logger.error(
                f"Errore durante l'ottenimento della versione locale: {stderr}"
            )
            return "err"
    ...
    def get_remote_version(self):
        ...
        if result.returncode != 0:
            logger.error(
                f"Errore durante l'ottenimento della versione remota: {result.stderr.decode('utf-8')}"
            )
            return "err"
    ...
    def get_release_notes(self):
        ...
        if result.returncode != 0:
            logger.error(
                f"Errore durante l'ottenimento delle note di rilascio: {result.stderr.decode('utf-8')}"
            )
    ...
    def check_update(self):
        logger.debug("Controllo aggiornamenti...")

        local_version = self.get_local_version()
        remote_version = self.get_remote_version()

        logger.debug(f"Versione locale: {local_version}")
        logger.debug(f"Versione remota: {remote_version}")

        # Handle error states
        if (
            local_version == "err"
            or remote_version == "err"
            or local_version == "untagged"
        ):
            self.aggiornamento_disponibile = False
            return False
        ...
        try:
            ...
            self.aggiornamento_disponibile = version.parse(
                remote_clean
            ) > version.parse(local_clean)
    ...
    def update(self):
        ...
        try:
            ...
            if result.returncode != 0:
                logger.error(
                    f"Errore durante l'aggiornamento: {result.stderr.decode('utf-8')}"
                )
    ...
    def auto_update(self):
        ...
        if self.check_update():
            logger.debug(
                "Aggiornamento disponibile, avviando processo di aggiornamento..."
            )
            try:
                self.update()
            except Exception as e:
